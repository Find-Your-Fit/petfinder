...
def preprocess(image_path, START,END,img_folder = "",max_size=512, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
    ...
    # imgs_meta = [aspectaware_resize_padding(cv2.normalize(img, None, 0, 255, cv2.NORM_MINMAX), max_size, max_size,
                                            # means=None) for img in tqdm.tqdm(ori_imgs)]
    # del ori_imgs
    # normalized_imgs = [(img[..., ::-1] / 255 - mean) / std for img in ori_imgs]
    # print("Image preprocessing")
    # imgs_meta = [aspectaware_resize_padding(img, max_size, max_size,
                                            # means=None) for img in tqdm.tqdm(normalized_imgs)]
